...
import { actionCartModal } from '@/modals.js'
...
const createAction = function () {
    ...
    api.post(route('actions.add-to-cart.store'), {
        actions: actions,
        product_code: productCode.value,
    })
        .then(() => {
...
<template>
    <ModalSlide
        ...
        :show="actionCartModal.show"
        @close="actionCartModal.close()"
    >
        <template #header> Добавление в корзину </template>

        <template #caption>
            <div class="flex items-center">
                <div>Укажите товар и запросы по которым товар будет добавлен в корзину.</div>
            ...
            <tr v-for="(action, index) in actions" :key="action.id" class="main-table__tr">
                ...
                <td>
                    <div class="flex justify-center">
                        <QuantityInput v-model="action.view_time" />
                    </div>
                </td>
                <td>
                    <div class="flex justify-center">
        ...
        <template #actions>
            <AppButton size="lg" :disabled="loading" @click="createAction">Отправить</AppButton>
        </template>
    </ModalSlide>
</template>
