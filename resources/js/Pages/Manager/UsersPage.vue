...
function loginAs(user) {
    form.post(route('manager.users.login-as', user.id))
}

const setPreference = (user) => {
    api.post(route('manager.users.set-preferences', { id: user.id }), {
            ...
            <div class="divide-y">
                ...
                >
                    ...
                    <div class="ml-auto flex gap-2">
                        ...
                        <Link :href="route('manager.users.show', { id: user.id })">
        ...
        <div class="pt-6 flex gap-2 justify-center">
            <div v-for="link in paginator" :key="link">
                <Link v-if="link.url" :href="link.url">
                    <AppButton v-html="link.label" :theme="link.active ? '' : 'normal'" />
                </Link>
                <AppButton v-else v-html="link.label" :theme="link.active ? '' : 'normal'" />
            </div>
        </div>
    </AuthenticatedLayout>
</template>
